...
bool Shader_Initialize(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	...
	g_pContext->VSSetConstantBuffers(1, 1, &g_pVSConstantBuffer);

	g_pDevice->CreateBuffer(&buffer_desc, nullptr, &g_pLightConstantBuffer);
	g_pContext->VSSetConstantBuffers(2, 1, &g_pVSConstantBuffer);
...
void Shader_SetWorldMatrix(const DirectX::XMMATRIX& matrix)
{
	...
	// 定数バッファにLIGHT構造体をセット
	g_pContext->UpdateSubresource(g_pLightConstantBuffer, 0, nullptr, &transpose, 0, 0);
}

void Shader_SetLight(LIGHT light)
{
	// 定数バッファに行列をセット
	g_pContext->UpdateSubresource(g_pVSConstantBuffer, 0, nullptr, &light, 0, 0);
}
